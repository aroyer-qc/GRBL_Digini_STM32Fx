...
void system_init()
{
	// With Digini pull up or not is done in bsp_io_def.h
	...
	// todo poutine interrupt init for the control pin
}
...
void system_reset_position(void)
{
...
uint8_t system_control_get_state()
{
	...
	uint8_t pin = ((IO_GetInputPin(IO_CONTROL_RESET) == 1 << CONTROL_RESET_BIT) |
				   (IO_GetInputPin(IO_CONTROL_FEED)  << CONTROL_FEED_BIT)       |
				   (IO_GetInputPin(IO_CONTROL_START) << CONTROL_CYCLE_START_BIT));
  #ifdef ENABLE_SAFETY_DOOR_INPUT_PIN
	pin |=          (IO_GetInputPinIO_SAFETY_DOOR) << CONTROL_SAFETY_DOOR_BIT));
  #endif
	...
	if (pin) {
      ...
      #else
		if(bit_istrue(pin, (1<<CONTROL_FEED_BIT))) { control_state |= CONTROL_PIN_INDEX_FEED_HOLD; }
      #endif
...
void system_convert_array_steps_to_mpos(float *position, const int32_t *steps)
{
	uint8_t idx;
	for(idx = 0; idx < N_AXIS; idx++) {
		position[idx] = system_convert_axis_steps_to_mpos((int32_t*)steps, idx);
	}
	return;
}
...
void system_set_exec_state_flag(uint16_t mask) {
    nOS_StatusReg   sr;
...
void system_clear_exec_state_flag(uint16_t mask) {
    nOS_StatusReg   sr;
    nOS_EnterCritical(sr);
	sys_rt_exec_state &= ~(mask);
    nOS_LeaveCritical(sr);
