...
void limit_pin_changeISR(void* pArg);
#ifdef ENABLE_SOFTWARE_DEBOUNCE
void limit_pin_debounceISR(TIM_TypeMatch_e TypeMatch);
#endif
...
//        IO_InitIRQ(IO_ISR_LIMIT_X, limit_pin_changeISR);
//        IO_EnableIRQ(IO_ISR_LIMIT_X);
      #ifdef GRBL_USE_AXIS_Y
//        IO_InitIRQ(IO_ISR_LIMIT_Y, limit_pin_changeISR);
//        IO_EnableIRQ(IO_ISR_LIMIT_Y);
      #endif
      #ifdef GRBL_USE_AXIS_Z
//        IO_InitIRQ(IO_ISR_LIMIT_Z, limit_pin_changeISR);
//        IO_EnableIRQ(IO_ISR_LIMIT_Z);
      #endif
      #ifdef GRBL_USE_AXIS_A
//        IO_InitIRQ(IO_ISR_LIMIT_A, limit_pin_changeISR);
//        IO_EnableIRQ(IO_ISR_LIMIT_A);
      #endif
      #ifdef GRBL_USE_AXIS_B
//        IO_InitIRQ(IO_ISR_LIMIT_B, limit_pin_changeISR);
//        IO_EnableIRQ(IO_ISR_LIMIT_B);
      #endif
        myTIM_GRBL_LimitsDebounce.RegisterCallBack(limit_pin_debounceISR);
...
void limits_disable()
{
	settings.system_flags &= ~BITFLAG_ENABLE_LIMITS;                // use for GUI
    IO_DisableIRQ(IO_ISR_LIMIT_X);
  #ifdef GRBL_USE_AXIS_Y
    IO_DisableIRQ(IO_ISR_LIMIT_Y);
  #endif
  #ifdef GRBL_USE_AXIS_Z
    IO_DisableIRQ(IO_ISR_LIMIT_Z);
  #endif
  #ifdef GRBL_USE_AXIS_A
    IO_DisableIRQ(IO_ISR_LIMIT_A);
  #endif
  #ifdef GRBL_USE_AXIS_B
    IO_DisableIRQ(IO_ISR_LIMIT_B);
  #endif
}
...
uint8_t limits_get_state()
{
	...
	pin =  IO_GetInputPin(IO_LIMIT_X) ? X_LIMIT_MASK : 0;
  #ifdef GRBL_USE_AXIS_Y
	pin |= IO_GetInputPin(IO_LIMIT_Y) ? Y_LIMIT_MASK : 0;
  #endif
  #ifdef GRBL_USE_AXIS_Z
    pin |= IO_GetInputPin(IO_LIMIT_Z) ? Z_LIMIT_MASK : 0;
  #endif
  #ifdef GRBL_USE_AXIS_A
    pin |= IO_GetInputPin(IO_LIMIT_A) ? A_LIMIT_MASK : 0;
  #endif
  #ifdef GRBL_USE_AXIS_B
    pin |= IO_GetInputPin(IO_LIMIT_B) ? B_LIMIT_MASK : 0;
  #endif

  #ifdef ENABLE_DUAL_AXIS
    pin |= IO_GetInputPin(IO_LIMIT_DUAL) ? DUAL_LIMIT_MASK : 0;             // DO Dual axis have limit switch????
  #endif
...
// This is the Limit Pin Change Interrupt callback, which handles the hard limit feature. A bouncing
// limit switch can cause a lot of problems, like false readings and multiple interrupt calls.
// If a switch is triggered at all, something bad has happened and treat it as such, regardless
// if a limit switch is being disengaged. It's impossible to reliably tell the state of a
// bouncing pin because the cpu does not retain any state information when
...
// your e-stop switch to the board reset pin, since it is the most correct way to do this.
void limit_pin_changeISR(void* pArg) // DEFAULT: Limit pin change interrupt process.
{
    VAR_UNUSED(pArg);
  ...
  #else
    myTIM_GRBL_LimitsDebounce.ReStart();              // Trigger the debounce (as long it is bouncing, we reload timer).
    IO_SetPinHigh(IO_DEBUG_PIN);
  #endif
}
