...
//note argo on arduino
// timer0			step pulse
// timer1			stepper driver
// timer2 			overflow something


// prototype
void Stepper_PortResetISR();
...
// ---------------------------------------------------------------------------- ARGO not in original GRBL 1.1H
...
// argo check this as it is not in advanced
  #else
...
typedef struct {
    ...
    uint8_t step_outbits;           // The next stepping-bits to be output      (include DUAL_AXIS bit if enable
    uint8_t dir_outbits;
  #ifdef ADAPTIVE_MULTI_AXIS_STEP_SMOOTHING
    uint32_t steps[N_AXIS];
  #endif
...
// Step and direction invert masks.
static uint8_t step_invert_mask;
static uint8_t dir_invert_mask;
...
void st_wake_up()
{
    ...
    if(bit_istrue(settings.flags, BITFLAG_INVERT_ST_ENABLE)) { IO_SetPinHigh(IO_ENABLE_DRIVE); }
    else { IO_SetPinLow(IO_ENABLE_DRIVE); }

    // Give steppers some time to wake up
    delay_ms(10);   // TODO Argo Why ST should need this????

    // Initialize stepper output bits to ensure first ISR call does not step.
    st.step_outbits = step_invert_mask;
  ...
  // Initialize step pulse timing from settings. Here to ensure updating after re-writing.
    ...
    // TIMER 0 on arduino // OCR0A = -(((settings.pulse_microseconds)*TICKS_PER_MICROSECOND) >> 3);
...
void st_go_idle()
{
    ...
    if(pin_state) { IO_SetPinHigh(IO_ENABLE_DRIVE); }
    else {  IO_SetPinLow(IO_ENABLE_DRIVE); }
}


/* "The Stepper Driver Callback" - This timer interrupt callback is the workhorse of Grbl.Grbl employs
   ...
   and maximizing the CPU capabilities. However, the downside of the Bresenham algorithm
   ...
   NOTE: This Callback expects at least one step to be executed per segment.
*/
// TODO: Replace direct updating of the int32 position counters in the ISR somehow. Perhaps use smaller
// int8 variables and update position counters only when a segment completes. This can get complicated
// with probing and homing cycles that require true real-time positions.
void Stepper_MainISR(TIM_TypeMatch_e TypeMatch)
{
    VAR_UNUSED(TypeMatch);
  ...
  #ifdef ENABLE_DUAL_AXIS
    if(st.dir_outbits & DUAL_DIRECTION_MASK) {
        IO_SetPinHigh(IO_DIR_DUAL_PORT);
    } else {
        IO_SetPinLow(IO_DIR_DUAL_PORT);
    }
  #endif
  #ifdef GRBL_USE_AXIS_Y
  ...
  #endif
  #ifdef GRBL_USE_AXIS_A
    if(st.dir_outbits & A_DIRECTION_MASK) {
  ...
  #endif
  #ifdef GRBL_USE_AXIS_B
    if(st.dir_outbits & B_DIRECTION_MASK) {
...
// poutine sync here 1 *************************************************************************************************************************************************************************
  ...
  #ifdef STEP_PULSE_DELAY
    st.step_bits = (STEP_PORT & ~STEP_MASK) | st.step_outbits; 		// Store out_bits to prevent overwriting. Include DUAL if ENABLE_DUAL_AXIS enable

// poutine sync here 2 *************************************************************************************************************************************************************************
#else
    if(st.step_outbits & X_STEP_MASK) {
        if(step_invert_mask & X_STEP_MASK) {
            // Low pulse
  ...
  #ifdef ENABLE_DUAL_AXIS
    if(st.step_outbits & DUAL_STEP_MASK) {
        if(step_invert_mask & DUAL_STEP_MASK) {
            // Low pulse
    ...
    if(st.step_outbits & Y_STEP_MASK) {
        if(step_invert_mask & Y_STEP_MASK) {
            // Low pulse
            IO_SetPinLow(IO_STEP_Y);
        } else {
            // High pulse
    ...
    if(st.step_outbits & Z_STEP_MASK) {
        if(step_invert_mask & Z_STEP_MASK) {
            // Low pulse
  ...
  #endif
  #ifdef GRBL_USE_AXIS_A
    if(st.step_outbits & A_STEP_MASK) {
        if(step_invert_mask & A_STEP_MASK) {
            // Low pulse
  ...
  #endif
  #ifdef GRBL_USE_AXIS_B
    if(st.step_outbits & B_STEP_MASK) {
        if(step_invert_mask & B_STEP_MASK) {
            // Low pulse
...
// poutine sync here 3 *************************************************************************************************************************************************************************
  ...
  // TCNT0 = st.step_pulse_time; // Reload Timer0 counter			// todo argo adapt			this is timer 0 on arduino
  // TCCR0B = (1<<CS01); // Begin Timer0. Full speed, 1/8 prescaler		//  todo argo adapt this is only configuration for what is written

	// sei(); // Re-enable interrupts to allow Stepper Port Reset Interrupt to fire on-time.   // TODO Argo to adapt not sure nesting is needed
    ...
    if(st.exec_segment == NULL) {
        ...
        if(segment_buffer_head != segment_buffer_tail) {
            ...
            // With AMASS is disabled, set timer prescaler for segments with slow step frequencies (< 250Hz).   timer 1
           // TCCR1B = (TCCR1B & ~(0x07<<CS10)) | (st.exec_segment->prescaler<<CS10);    // todo argo.... configure for ???
			// argo equivalent to MODIFY_REG(timer, (0x07 << XX_pos),  prescaler << XX_Pos);
          #endif
...
// poutine sync here 4 *************************************************************************************************************************************************************************
            ...
            if(st.exec_block_index != st.exec_segment->st_block_index) {
              ...
              #ifdef GRBL_USE_AXIS_Y
                st.counter_y = (st.exec_block->step_event_count >> 1);
              #endif
              #ifdef GRBL_USE_AXIS_Z
              ...
              #endif
              #ifdef GRBL_USE_AXIS_B
                st.counter_b = (st.exec_block->step_event_count >> 1);
              #endif
            }
// poutine sync here 5 *************************************************************************************************************************************************************************

            st.dir_outbits = st.exec_block->direction_bits ^ dir_invert_mask;
...
// poutine sync here 6 *************************************************************************************************************************************************************************
    ...
    if(st.counter_x > st.exec_block->step_event_count) {
        ...
        st.step_outbits_dual = DUAL_STEP_MASK;
    #endif
        st.counter_x -= st.exec_block->step_event_count;

        if(st.exec_segment->backlash_motion == 0) {
            if(st.exec_block->direction_bits & X_DIRECTION_MASK) { sys_position[X_AXIS]--; }
    ...
    if(st.counter_y > st.exec_block->step_event_count) {
      ...
      st.step_outbits_dual = DUAL_STEP_MASK;
        ...
        if(st.exec_segment->backlash_motion == 0) {
            if(st.exec_block->direction_bits & Y_DIRECTION_MASK) { sys_position[Y_AXIS]--; }
    ...
    if(st.counter_z > st.exec_block->step_event_count) {
        ...
        if(st.exec_segment->backlash_motion == 0) {
            if(st.exec_block->direction_bits & Z_DIRECTION_MASK) { sys_position[Z_AXIS]--; }
    ...
    if(st.counter_a > st.exec_block->step_event_count) {
        st.step_outbits |= A_STEP_MASK;
        st.counter_a -= st.exec_block->step_event_count;

        if(st.exec_segment->backlash_motion == 0) {
            if(st.exec_block->direction_bits & A_DIRECTION_MASK) { sys_position[A_AXIS]--; }
    ...
    if(st.counter_b > st.exec_block->step_event_count) {
        st.step_outbits |= B_STEP_MASK;
        st.counter_b -= st.exec_block->step_event_count;

        if(st.exec_segment->backlash_motion == 0)
        {
            if(st.exec_block->direction_bits & B_DIRECTION_BIT) { sys_position[B_AXIS]--; }
...
// poutine sync here 7 *************************************************************************************************************************************************************************
  ...
  st.step_outbits ^= step_invert_mask;  // Apply step port invert mask
  #ifdef ENABLE_DUAL_AXIS
...
void Stepper_PortResetISR(void)
{
    ...
    if(step_invert_mask & X_STEP_MASK) { IO_SetPinHigh(IO_STEP_X); }
	else { IO_SetPinLow(IO_STEP_X); }

 #ifdef ENABLE_DUAL_AXIS
    if(step_invert_mask & DUAL_STEP_MASK) { IO_SetPinHigh(IO_STEP_DUAL); }
	else { IO_SetPinLow(IO_STEP_DUAL); }
  #endif

  #ifdef GRBL_USE_AXIS_Y
    if(step_invert_mask & Y_STEP_MASK) { IO_SetPinHigh(IO_STEP_Y); }
	else { IO_SetPinLow(IO_STEP_Y); }
  #endif

  #ifdef GRBL_USE_AXIS_Z
    if(step_invert_mask & Z_STEP_MASK) { IO_SetPinHigh(IO_STEP_Z); }
	else { IO_SetPinLow(IO_STEP_Z); }
  #endif

  #ifdef GRBL_USE_AXIS_A
    if(step_invert_mask & A_STEP_MASK) { IO_SetPinHigh(IO_STEP_A); }
    else { IO_SetPinLow(IO_STEP_A); }
  #endif

  #ifdef GRBL_USE_AXIS_B
    if(step_invert_mask & B_STEP_MASK) { IO_SetPinHigh(IO_STEP_B); }
	else { IO_SetPinLow(IO_STEP_B); }
  #endif
}
  ...
  // st_wake_up() routine.
void st_pulse_delayISR(void)
{
    // Begin step pulse.
    if(st.step_bits & X_STEP_MASK) { IO_SetPinHigh(IO_STEP_X); }
 #ifdef ENABLE_DUAL_AXIS
    if(st.step_bits & DUAL_STEP_MASK) { IO_SetPinHigh(IO_STEP_DUAL); }
  #endif
  #ifdef GRBL_USE_AXIS_Y
    if(st.step_bits & Y_STEP_MASK) { IO_SetPinHigh(IO_STEP_Y); }
  #endif
  #ifdef GRBL_USE_AXIS_Z
    if(st.step_bits & Z_STEP_MASK) { IO_SetPinHigh(IO_STEP_Z); }
  #endif
  #ifdef GRBL_USE_AXIS_A
    if(st.step_bits & A_STEP_MASK) { IO_SetPinHigh(IO_STEP_A); }
  #endif
  #ifdef GRBL_USE_AXIS_B
    if(st.step_bits & B_STEP_MASK) { IO_SetPinHigh(IO_STEP_B); }
  #endif
  }
#endif
...
void st_generate_step_dir_invert_masks()
{
    ...
    for(idx = 0; idx < N_AXIS; idx++) {
        if(bit_istrue(settings.step_invert_mask, bit(idx))) { step_invert_mask |= get_step_pin_mask(idx); }
        if (bit_istrue(settings.dir_invert_mask,bit(idx))) { dir_invert_mask |= get_direction_pin_mask(idx); }
    }
  #ifdef ENABLE_DUAL_AXIS
    step_invert_mask_dual = 0;
    dir_invert_mask_dual = 0;
    // NOTE: Dual axis invert uses the N_AXIS bit to set step and direction invert pins.
    if (bit_istrue(settings.step_invert_mask,bit(N_AXIS))) { step_invert_mask_dual = (1<<DUAL_STEP_MASK); }
    if (bit_istrue(settings.dir_invert_mask,bit(N_AXIS))) { dir_invert_mask_dual = (1<<DUAL_DIRECTION_BIT); }
  #endif
}
...
void st_reset()
{
    ...
    // st.exec_segment = NULL; // already set by memset
    ...
    st.dir_outbits = dir_invert_mask; // Initialize direction bits to default.
    ...
    IO_SetPinLow(IO_STEP_X);
  #ifdef ENABLE_DUAL_AXIS
    IO_SetPinLow(IO_STEP_DUAL_AXIS);
  #endif
  #ifdef GRBL_USE_AXIS_Y
    IO_SetPinLow(IO_STEP_Y);
  #endif
...
void stepper_init()
{
    // Note Timer module is already initialize by class constructor
	// Configuration of the timer found in the tim_cfg.h file
	myTIM_GRBL_Stepper.RegisterCallBack(Stepper_MainISR);                      // Register ISR Callback
}
...
void st_prep_buffer()
{
    ...
    while(segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
      ...
      #if 0 // commented, this is on arduino code ARGO
        if (cycles < (1UL << 16)) { prep_segment->cycles_per_tick = cycles; } // < 65536 (4.1ms @ 16MHz)
        else { prep_segment->cycles_per_tick = 0xffff; } // Just set the slowest speed possible.
      #endif
     #else
        // Compute step timing and timer prescalar for normal step generation.
        if(cycles < (1UL << 16)) { // < 65536 (2.7ms @ 24MHz)
          #if 0 // commented, this is on arduino code ARGO
          ...
          #if 0 // commented , this ison arduino code ARGO
            }
          #endif
        }
    #endif
