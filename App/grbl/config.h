...
// TODO (Alain#2#): change this so it use define axis in IO
...
#define N_DECIMAL_COORD_INCH_STR	    "4" // Coordinate or position value in inches
#define N_DECIMAL_COORD_MM_STR  	    "3" // Coordinate or position value in mm
#define N_DECIMAL_RATE_INCH_STR 	    "1" // Rate or velocity value in in/min
#define N_DECIMAL_RATE_MM_STR   	    "0" // Rate or velocity value in mm/min
#define N_DECIMAL_SETTING_STR   	    "3" // Decimals for floating point setting values
#define N_DECIMAL_RPM_STR    	        "0" // Decimal string config for RPM value in rotations per min.
#define N_DECIMAL_RPM_VALUE    	         0  // Decimal config RPM value in rotations per min.
...
// Grbl combines the variable spindle PWM and the enable into one pin to help
// preserve I/O pins. For certain setups, these may need to be separate pins. This configure option uses
// the spindle direction pin as a separate spindle enable pin along with spindle speed PWM.
...
// #define USE_SPINDLE_DIRECTION_PIN // Default disabled. Uncomment to enable.
...
// monitoring the hard limit switch pins will enable a timer on the CPU to re-check
// the limit pin state after a delay of specified delay in mSec. This can help with CNC machines
// with problematic false triggering of their hard limit switches, but it WILL NOT fix issues with
// electrical interference on the signal cables from external sources. It's recommended to first
// use shielded signal cables with their shielding connected to ground (old USB/computer cables
// work well and are cheap to find) and wire in a low-pass circuit into each limit pin.
#define ENABLE_SOFTWARE_DEBOUNCE        // Default disabled. Uncomment to enable.
#define DEBOUNCE_TIME_CHECK         32  // Value is in mSec
