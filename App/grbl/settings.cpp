...
void settings_restore(uint8_t restore_flag) {
	if(restore_flag & SETTINGS_RESTORE_DEFAULTS) {
		...
		settings.backlash    [A_AXIS] = DEFAULT_A_BACKLASH;
		...
		settings.backlash    [B_AXIS] = DEFAULT_B_BACKLASH;
...
uint8_t get_step_pin_mask(uint8_t axis_idx)
{
	...
	if(axis_idx == Z_AXIS) { return (Z_STEP_MASK); }
  #endif
  #ifdef GRBL_USE_AXIS_A
	if(axis_idx == A_AXIS) { return (A_STEP_MASK); }
  #endif
  #ifdef GRBL_USE_AXIS_B
	if(axis_idx == B_AXIS) { return (B_STEP_MASK); }
  #endif
...
uint8_t get_direction_pin_mask(uint8_t axis_idx)
{
  ...
  #endif
  #ifdef GRBL_USE_AXIS_Z
	if(axis_idx == Z_AXIS) { return (Z_DIRECTION_MASK); }
  #endif
  #ifdef GRBL_USE_AXIS_A
	if(axis_idx == A_AXIS) { return (A_DIRECTION_MASK); }
  #endif
  #ifdef GRBL_USE_AXIS_B
	if(axis_idx == B_AXIS) { return (B_DIRECTION_MASK); }
  #endif
...
{
	if(axis_idx == X_AXIS) { return (X_LIMIT_MASK); }
  #ifdef GRBL_USE_AXIS_Y
	if(axis_idx == Y_AXIS) { return (Y_LIMIT_MASK); }
  #endif
  #ifdef GRBL_USE_AXIS_Z
	if(axis_idx == Z_AXIS) { return (Z_LIMIT_MASK); }
  #endif
  #ifdef GRBL_USE_AXIS_A
	if(axis_idx == A_AXIS) { return (A_LIMIT_MASK); }
  #endif
  #ifdef GRBL_USE_AXIS_B
	if(axis_idx == B_AXIS) { return (B_LIMIT_MASK); }
  #endif
